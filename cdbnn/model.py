...
class SubtleDetailCNN(nn.Module):
    def __init__(self, in_channels, num_classes, input_size=(224, 224)):
        ...
        self.fc1 = nn.Linear(self.feature_size, 32)  # Changed to 32 features
        self.fc2 = nn.Linear(32, num_classes)  # Adjusted to take 32 features as input
    ...
    def forward(self, x):
        ...
        if x.shape[1] != self.feature_size:
            ...
            self.fc1 = nn.Linear(x.shape[1], 32).to(x.device)  # Changed to 32 features
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        try:
            ...
            if "size mismatch" in str(e):
                ...
                self.fc1 = nn.Linear(state_dict['fc1.weight'].shape[1], 32).to(next(self.parameters()).device)  # Changed to 32 features
...
class InverseSubtleDetailCNN(nn.Module):
    def __init__(self, in_channels, num_classes, output_size=(224, 224)):
        ...
        self.feature_size = 32 * h * w  # Changed to 32 features

        self.fc1 = nn.Linear(num_classes, self.feature_size)  # Adjusted to take 32 features as input
        self.deconv1 = nn.ConvTranspose2d(32, 1024, kernel_size=3, stride=1, padding=1)  # Changed to 32 features
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 32, self.output_h, self.output_w)  # Changed to 32 features
...
class SubtleDetailCNN128(nn.Module):
    def __init__(self, in_channels, num_classes, input_size=(224, 224)):
        super(SubtleDetailCNN, self).__init__()
        self.conv1 = nn.Conv2d(in_channels, 32, kernel_size=3, stride=1, padding=1)
        self.conv2 = nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1)
    ...
    def load_state_dict128(self, state_dict, strict=True):
        """Override load_state_dict to handle size mismatches in fc1."""
        try:
            super().load_state_dict(state_dict, strict)
        except RuntimeError as e:
...
class InverseSubtleDetailCNN128(nn.Module):
    def __init__(self, in_channels, num_classes, output_size=(224, 224)):
        super(InverseSubtleDetailCNN, self).__init__()
        # Calculate feature size based on output dimensions
        h, w = output_size
